...
void Intersection::addVehicleToQueue(std::shared_ptr<Vehicle> vehicle)
{
    ...
    lck.lock();
    std::cout << "Intersection #" << _id << ": Vehicle #" << vehicle->getID() << " is granted entry checking traffic light." << std::endl;
    lck.unlock();
    // FP.6b : use the methods TrafficLight::getCurrentPhase and TrafficLight::waitForGreen to block the execution until the traffic light turns green.
     ...
     // Check the traffic light phase before entering the intersection   
    if(_trafficLight.getCurrentPhase() == TrafficLightPhase::red){
        std::cout << "Intersection #" << _id << ": Vehicle #" << vehicle->getID() << " waiting as traffic light red." << std::endl;
        _trafficLight.waitForGreen();
    }
    std::cout << "Intersection #" << _id << ": Vehicle #" << vehicle->getID() << " is granted entry as traffic light green." << std::endl;
